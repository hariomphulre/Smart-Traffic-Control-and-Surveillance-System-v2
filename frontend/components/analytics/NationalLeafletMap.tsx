...
// 🔥 STRICT INDIA BOUNDARY HIGHLIGHTER 🔥
const MapBoundsUpdater = ({ signals }: { signals: any[] }) => {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    // Zoom Map to all signals if they exist
    if (signals && signals.length > 0) {
      const bounds = L.latLngBounds(signals.map(s => [s.lat, s.lng]));
      map.flyToBounds(bounds, { padding: [50, 50], maxZoom: 10, duration: 1.5 });
    } 
    
    // Fetch Border Boundary strictly for India
    const fetchNationalBounds = async () => {
      try {
        const res = await fetch(`https://nominatim.openstreetmap.org/search?country=India&format=json&polygon_geojson=1`);
        ...
        if (data && data.length > 0 && isMounted) {
          ...
          // If there are literally no signals saved yet, fallback to zooming strictly to India's bounding box
          if (!signals || signals.length === 0) {
            const bbox = data[0].boundingbox;
            map.flyToBounds(
              [ [parseFloat(bbox[0]), parseFloat(bbox[2])], [parseFloat(bbox[1]), parseFloat(bbox[3])] ], 
              { padding: [20, 20], maxZoom: 5, duration: 1.5 }
            );
          }
        }
      } catch (err) { 
        console.error('Failed to geocode national bounds:', err); 
      }
    };
  ...
  return (
    <GeoJSON 
      ...
      interactive={false} 
      style={{
        color: '#8AB4F8', // Primary Blue border
        weight: 2.5,
        opacity: 0.8,
        fill: false // Removes background tint completely, just clean lines
      }}
    />
  );
};

export interface NationalLeafletMapProps {
  signals: any[];
  onSquareClick?: (id: string) => void;
}

export default function NationalLeafletMap({ signals, onSquareClick }: NationalLeafletMapProps) {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <MapContainer center={defaultCenter} zoom={5} style={{ height: '100%', width: '100%', background: '#0a0f14' }} zoomControl={true}>
        
        <ZoomListener onZoomChange={setCurrentZoom} />
        <MapBoundsUpdater signals={signals} />
        ...
        {signals.map((signal) => {
          ...
          return (
            <React.Fragment key={signal.id}>
              ...
              {/* 3. Way Labels (R1, R2, R3) - ONLY VISIBLE IF ZOOMED IN (zoom >= 16) */}
              ...
              {/* 4. Central Signal Node (Always visible) */}
              <Marker
