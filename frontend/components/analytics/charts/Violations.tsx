...
const VIEW_TITLES: Record<ViolationView, string> = {
  ...
  redLightJump: 'Red Light Jump Violations',
  ...
  return (
    <div
      ...
      className={`flex font-sans bg-[#131314] text-gray-200 relative z-30 isolate ${
        isFullscreen ? 'h-screen w-screen p-6' : 'h-140 w-full border-t border-b border-r border-[#3c4043] overflow-hidden shadow-2xl'
      }`}
    >
      <div className="w-1/5 min-w-[200px] border-r border-[#3c4043] p-4 flex flex-col gap-4 overflow-y-auto bg-[#131314] z-10">
        <ChartThumbnail
          ...
          lines={overviewMetrics}
        />
        <ChartThumbnail
          view="speed"
          title="Speed"
          lines={buildMetrics('speed', activeWays)}
        />
        <ChartThumbnail
          view="helmet"
          title="Helmet"
          lines={buildMetrics('helmet', activeWays)}
        />
        <ChartThumbnail
          view="tripling"
          title="Tripling"
          lines={buildMetrics('tripling', activeWays)}
        />
        <ChartThumbnail
          view="redLightJump"
          title="Red Light Jump"
          lines={buildMetrics('redLightJump', activeWays)}
        />
      </div>
