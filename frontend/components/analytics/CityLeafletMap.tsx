...
import { MapContainer, TileLayer, CircleMarker, Polyline, Marker, useMap, useMapEvents } from 'react-leaflet';
  ...
  return L.divIcon({
    ...
    html: `
      <div style="display:flex;flex-direction:column;align-items:center;transform:translateY(-18px);">
        <!-- Flat Google Maps Style Pin SVG (Transparent Hole) -->
...
const MapBoundsUpdater = ({ currentCity, signals }: { currentCity: string, signals: any[] }) => {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    if (signals && signals.length > 0) {
      ...
      const fetchCityBounds = async () => {
        try {
          const res = await fetch(`https://nominatim.openstreetmap.org/search?city=${encodeURIComponent(currentCity)}&format=json`);
          ...
          console.error('Failed to geocode city bounds:', err); 
        }
      };
      fetchCityBounds();
    }
  }, [currentCity, signals, map]);

  return null;
};

export interface CityLeafletMapProps {
  signals: any[];
  currentCity: string;
  onSquareClick?: (id: string) => void;
}

export default function CityLeafletMap({ signals, currentCity, onSquareClick }: CityLeafletMapProps) {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <MapContainer center={defaultCenter} zoom={13} style={{ height: '100%', width: '100%', background: '#0a0f14' }} zoomControl={true}>
        
        <ZoomListener onZoomChange={setCurrentZoom} />
        
        <MapBoundsUpdater currentCity={currentCity} signals={signals} />
        ...
        {signals.map((signal) => {
          ...
          return (
            <React.Fragment key={signal.id}>
              ...
              {/* 4. Central Signal Node (Always visible, #8AB4F8 Color Pin) */}
              <Marker
